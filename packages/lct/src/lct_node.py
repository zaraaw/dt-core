...
class LCTNode(DTROS):
    ...
    def lanepose_callback(self, msg_lane_pose):
        ...
        d = round(abs(msg_lane_pose.d), 2)
        phi = round(abs(msg_lane_pose.phi), 2)
        ...
        if state == "NORMAL_JOYSTICK_CONTROL":
            ...
            lct_out, rwd = LCT.runLCT().run_LCT(d, phi, v)
            ...
            if lct_out == None:                                             # keeps gain the same if no action selected from lct
                ...
                gain = lct_out
            """
            m = 0.38666                                                       # scales trim to new gain 
            c = 0.51333
            trim = m*gain + c
            """           

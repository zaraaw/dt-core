...
class Reward:
    ...
    def calc_reward(self, d, phi, v):                       # for gain as v 
        ...
        return reward   
    """
    def calc_reward(self, d, phi, v):                       # for car cmd v as v
        # slow speed is high v
        if self.low_v < v <= self.max_v and (d <= self.d1 or d >= self.d4) and (phi <= self.phi1 or phi >= self.phi4):
            print("punished for bad deviation - d & phi and slow speed ")
            reward = 0.01
        ...
        # fast speed is low v
        elif v < self.low_v and (d <= self.d1 or d <= self.d4) and (self.phi2 <= phi <= self.phi3):
            print("rewarded for going straight - d & phi and fast speed ")
            reward = 1
